...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                        ...
                        if(frame_idx>(N_BEFORE)):
                            ...
                            if(mod==0):
                                ...
                                if(id in refpoint[frame_prev]):
                                    ...
                                    label=f'{id} v:{y_speed[frame_idx][id]:.2f} va:{abs_speed[frame_idx][id]:.2f}'
                                else:
                                    label=f'{id} ...'
                            else:
                                if(id in y_speed[frame_idx-mod]):
                                    label=f'{id} v:{y_speed[frame_idx-mod][id]:.2f} va:{abs_speed[frame_idx-mod][id]:.2f}'
                                else:
                                    label=f'{id} ...'
                        else:
                ...
                dt[4]=t7-t6
                if(frame_idx%N_JUMP==0):
                    LOGGER.info(f'{s}Done. YOLO:({t3 - t2:.3f}s) | DeepSort:({t5 - t4:.3f}s) | SpeedEstm: ({t7-t6:.3f})')
                else:
                    LOGGER.info(f'{s}Skip. YOLO:(0s) | DeepSort:(0s) | SpeedEstm:(0s)')
            else:
    ...
    LOGGER.info(f'Speeds: %.1fms pre-process | %.1fms inference | %.1fms NMS | %.1fms deep sort update |\
speed estimation %.1fms | per image at shape {(1, 3, *imgsz)}' % t)
